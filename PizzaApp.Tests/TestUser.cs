...
{
    public SignedInUser? User { get; set; } = new(new TestUser().Id, "admin@example.test", true);
    public event Action? Changed;
    public Task SignInAsync(string email, string password)
    { User = new(new TestUser().Id, email, false); Changed?.Invoke(); return Task.CompletedTask; }
